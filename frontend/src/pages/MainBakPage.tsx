        ...
        <MainLayout>
            <Space direction="vertical" size="large" className={styles.container}>
                <Card title="보고서 생성" bordered={false}>
                    <Form form={form} name="reportForm" onFinish={onFinish} layout="vertical">
                        <Form.Item
                            label="보고서 주제"
                            name="topic"
                            rules={[
                            ...
                            <Input placeholder="예: 2025년 디지털 뱅킹 서비스 현황 분석" size="large" />
                        </Form.Item>

                        <Form.Item label="필수 내용" name="requiredContent" tooltip="보고서에 반드시 포함되어야 할 내용을 입력하세요 (선택사항)">
                            <TextArea
                                rows={4}
                                placeholder="예:
- 2024년 대비 성장률 분석
...
- 향후 3년간 예상 시장 규모"
                        ...
                        <Form.Item label="참조 웹사이트" tooltip="보고서 생성 시 참조할 웹사이트 주소를 입력하세요 (선택사항)">
                                        ...
                                        {fields.map(({key, name, ...restField}) => (
                                            <div key={key} className={styles.urlInputContainer}>
                                                <Form.Item
                                                    ...
                                                    <Input placeholder="예: https://www.example.com" />
                                                </Form.Item>
                                                <MinusCircleOutlined onClick={() => remove(name)} className={styles.removeIcon} />
                                            </div>
                                        ))}
                        ...
                        <Form.Item>
                            <Button type="primary" htmlType="submit" loading={isGenerating} size="large" block>
